...
const College: React.FC<CollegeProps> = ({name, description, logo}) => {
		return (
			...
			{window.innerWidth > 600 && <p> {description.length > 100?description.substring(0, 100) + '...':description}</p>}
			{window.innerWidth <= 600 && <p>{description.length > 40?description.substring(0, 40) + '...':description}</p>}
			</div>
			</div>
			</a>
			</Link>
