...
const CreateEvent: React.FC<EventProps> = ({id, event, UTCOffset}) => {
	...
	const formSubmit = async (e:any) => {
		...
		router.push(`/${router.query['community'] || context.editableData?.collegeName}/events`)
	}
  	...
  	if (file) {
		...
		return (<div>
			...
			<div id={styles.title}>{event? "Edit Event" : "Create Event"}
				<div className={styles.cancel} onClick={() => {router.push(`/${router.query['community'] || context.editableData?.collegeName}/events`); context.setEdit({})}}> Cancel</div>
			</div>
			</div>
			...
			<form className={styles.formBody} onSubmit={formSubmit}>
				...
				<label>
					...
					<p>Enter the event image here. Max file size is 2 MB.</p>
